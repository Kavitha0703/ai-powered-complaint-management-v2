...
const dynamicI18nStart = code.indexOf('// Dynamic i18n Translation Endpoint');
if (chatStart !== -1 && dynamicI18nStart !== -1) {
...
\n`;
    code = code.substring(0, chatStart) + newChatHandler + code.substring(dynamicI18nStart);
    fs.writeFileSync('api/_app.ts', code);
    console.log("Successfully patched api/_app.ts");
} else {
