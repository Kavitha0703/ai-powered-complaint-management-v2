...
// Update title for Current Admins
content = content.replace(
  /\{"Current Admins \("\}\{combinedAdminsList\.length\}\)/,
  `{"Current Admins ("}{currentAdmins.length})`
);

// Append a Pending Invitations section after the Current Admins section
// Find the end of the Current Admins section.
// It ends with: "No administrators match your current query or filter criteria."}</span>\n                </div>\n              </div>\n            )}\n          </div>\n        </div>\n      </div>"

const insertionPoint = `          </div>\n        </div>\n      </div>`;
const parts = content.split(insertionPoint);

if (parts.length === 2) {
  const pendingSection = `          </div>
        </div>
  ...
  content = parts[0] + pendingSection + parts[1];
}
