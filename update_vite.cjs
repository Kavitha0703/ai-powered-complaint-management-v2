const fs = require('fs');

let config = fs.readFileSync('vite.config.ts', 'utf8');

const newIcons = `
          icons: [
            { src: "/favicon-16x16.png", sizes: "16x16", type: "image/png" },
            { src: "/favicon-32x32.png", sizes: "32x32", type: "image/png" },
            { src: "/favicon-48x48.png", sizes: "48x48", type: "image/png" },
            { src: "/logo-72.png", sizes: "72x72", type: "image/png" },
            { src: "/logo-96.png", sizes: "96x96", type: "image/png" },
            { src: "/logo-128.png", sizes: "128x128", type: "image/png" },
            { src: "/logo-144.png", sizes: "144x144", type: "image/png", purpose: "any" },
            { src: "/logo-152.png", sizes: "152x152", type: "image/png", purpose: "any" },
            { src: "/logo-167.png", sizes: "167x167", type: "image/png", purpose: "any" },
            { src: "/apple-touch-icon.png", sizes: "180x180", type: "image/png", purpose: "any" },
            { src: "/logo-192.png", sizes: "192x192", type: "image/png", purpose: "maskable any" },
            { src: "/logo-256.png", sizes: "256x256", type: "image/png", purpose: "any" },
            { src: "/logo-384.png", sizes: "384x384", type: "image/png", purpose: "any" },
            { src: "/logo-512.png", sizes: "512x512", type: "image/png", purpose: "maskable any" },
            { src: "/logo-1024.png", sizes: "1024x1024", type: "image/png", purpose: "any" }
          ]
`;

config = config.replace(/icons: \[\s*\{[^\]]*\]/, newIcons.trim());
fs.writeFileSync('vite.config.ts', config);
