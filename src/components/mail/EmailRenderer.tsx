import { EmailBlock } from "./EmailEditor";
...
export function renderEmailHtml(blocks: EmailBlock[], templateVars: Record<string, string> = {}) {
  ...
  const blockHtmls = blocks.map(block => {
    ...
    switch (block.type) {
      case 'heading':
        return `<h1 style="font-size: 20px; color: #111827; font-weight: 700; margin-top: 0; margin-bottom: 16px; line-height: 1.3;">${content}</h1>`;
      case 'paragraph':
        return `<p style="font-size: 15px; color: #111827; line-height: 1.6; margin-top: 0; margin-bottom: 16px;">${content}</p>`;
      case 'button':
        const url = replaceVars(block.metadata?.url || "#");
        return `<div style="margin: 24px 0;"><a href="${url}" style="display: inline-block; background-color: ${BRAND_COLOR}; color: #ffffff; text-decoration: none; font-weight: 500; padding: 12px 24px; border-radius: 6px; font-size: 14px; text-align: center;">${content}</a></div>`;
      case 'card':
        const title = replaceVars(block.metadata?.title || "");
        return `<div style="border-top: 1px solid #E5E7EB; border-bottom: 1px solid #E5E7EB; padding: 16px 0; margin: 24px 0;">
          ...
          <div style="font-size: 14px; color: #111827; line-height: 1.6; white-space: pre-wrap;">${content}</div>
        </div>`;
      case 'divider':
        return `<hr style="border: none; border-top: 1px solid #E5E7EB; margin: 24px 0;" />`;
      case 'signature':
        return `<div style="margin-top: 32px; font-size: 14px; color: #6B7280; line-height: 1.5;">${content}</div>`;
      default:
        return "";
    }
  }).join('\n');
