...
export default function AuthPage({ isAdmin }: { isAdmin?: boolean }) {
  ...
  // Check for invite token or prefilled email from URL
  React.useEffect(() => {
    ...
    if (modeParam === "register" || tokenParam) setActiveTab("register");

    if (tokenParam || emailParam) {
      const invites = getAdminInvites();
      const inv = invites.find(i => 
        (tokenParam && i.token === tokenParam) || 
        (emailParam && i.email.toLowerCase() === emailParam.toLowerCase())
      );
      if (inv) {
        if (inv.expires_at && new Date(inv.expires_at) < new Date()) {
          setError("⚠️ This invitation token has expired. Please ask a Super Admin to send a new invitation.");
        } else {
          setMsg(`✅ Security Invitation Verified for ${inv.email} (${inv.role.replace('_', ' ')}). Complete account registration below.`);
        }
      }
    }
  }, []);
    ...
    try {
      ...
      if (error) {
        ...
        if (pendingIdx !== -1) {
          ...
          setMsg("Administrator registration complete! Your account is now active and ready for sign in.");
        } else {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      setError("");
      setMsg("");
      const result = await signInWithGoogle({ isAdmin });
      if (!result.success) {
        setError(result.error || "Failed to sign in with Google.");
      } else {
        setMsg("Redirecting to Google Authentication account chooser...");
        if (result.data?.url) {
              ...
              {activeTab !== "forgot" && (
                <div className="mt-5 pt-4 border-t border-slate-800/80 space-y-3">
                  <div className="relative flex items-center justify-center">
                    <span className="bg-slate-900/90 px-3 text-[10px] uppercase tracking-widest font-black text-slate-500">
                      {"OR"}
                    </span>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={loading}
                    className="w-full bg-slate-950 hover:bg-slate-850 text-slate-200 border-slate-800 hover:border-slate-700 font-bold text-xs h-10 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                  >
                    ...
                    <span>{isAdmin ? "Continue with Google (Admin)" : "Continue with Google"}</span>
                  </Button>
                </div>
              )}
